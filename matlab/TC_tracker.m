...
A = textscan(fileID,'%d %d %d %d %d %d %f %d %d %d %d','Delimiter',',');
fclose(fileID);
...
%     right_pts(:,2) = min(right_pts(:,2),track_params.img_size(1));
    ...
    track_struct.track_obj{n}.det_score = M(idx,7);
...
for t = 1:track_params.num_fr
    ...
    for n = 1:size(track_struct.tracklet_mat.xmin_mat,1)
        ...
        text(x_min,y_min-20,num2str(n),'FontSize',20,'Color', rand_color(n,:))
         % Plot confidence, class
    
        det_class = track_struct.tracklet_mat.class_mat{n,t};  % Lấy det_class của đối tượng n tại khung hình t
        det_score = track_struct.tracklet_mat.det_score_mat(n, t);  % Lấy det_score của đối tượng n tại khung hình t
        if det_class == 0
            class_str = 'auto';
        elseif det_class == 1
            class_str = 'bus';
        ...
        text_str = ['Class: ', class_str, ', Score: ', sprintf('%.2f', det_score)];
        text(x_min, y_max + 20, text_str, 'FontSize', 12, 'Color', 'yellow');
    end
